...
( function() {
	...
	function loadPlugins( editor ) {
		...
		CKEDITOR.plugins.load( plugins.split( ',' ), function( plugins ) {
            ...
            var getByDependencies = function(plugin, plugins, hadBeenOrdered) {
                var result = [];

                if (!hadBeenOrdered[plugin.name]) {
                    //loop over required plugins
                    ...
                    if (requires) {
                        ...
                        for (var i=0; i<requires.length; i++) {
                            var requiredPlugin = plugins[requires[i]];
                            var orderedRequired = getByDependencies(requiredPlugin, plugins, hadBeenOrdered);
                            result = result.concat(orderedRequired);
                        }
                    }

                    hadBeenOrdered[plugin.name] = true;
            ...
            var orderedPlugins = []; //A plugin with dependencies will appear after the dependency plugins in the array
            var hadBeenOrdered = [];
            for ( var pluginName in plugins ) {
                var pluginDependencies = getByDependencies(plugins[ pluginName ], plugins, hadBeenOrdered);
                orderedPlugins = orderedPlugins.concat(pluginDependencies);
            }

//
// this is a unit test function to make sure dependency order is preserved when initializing plugins
//
// var isDependencyOrderValid = function(orderedPluginsArray){
//                var loadedPlugins = {};
//
//                for(var i=0; i<orderedPluginsArray.length; i++){
//                    var currentPlugin = orderedPluginsArray[i];
//                    var requires = currentPlugin.requires;
//
//                    loadedPlugins[currentPlugin.name] = true;
//                    if (requires){
//                        if(!CKEDITOR.tools.isArray( requires )) {
//                            requires = requires.split(",");
//                        }
//                        for(var j=0; j<requires.length; j++){
//                            if(!loadedPlugins.hasOwnProperty(requires[j]))
//                                return false;
//                        }
//                    }
//                }
...
//            };
			...
			for ( var ind=0; ind< orderedPlugins.length; ind++ ) {
                var plugin = orderedPlugins[ ind ];
