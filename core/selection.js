...
( function() {
    ...
    //@author Alissa do not remove filling filling char flag
	...
	function getOnKeyDownListener( editor ) {
		...
		return function( evt ) {
			...
			if ( next && next.type == CKEDITOR.NODE_ELEMENT && next.getAttribute( 'contenteditable' ) == 'false' ) {
                //@author Noam - delete empty line before or after widget - if you do delete
	...
	CKEDITOR.dom.selection.prototype = {
		...
		getNative: function() {
            ...
            /** @author eran- use legacy code for firefox */
			if (CKEDITOR.env.gecko){
			...
			if ( isMSSelection ) {
				...
				for ( var i = 0; i < ranges.length; i++ ) {
                    ...
                    if ( range.collapsed && CKEDITOR.env.webkit && rangeRequiresFix( range ) ) {
                      ...
                      /**  @author etai: commented out. No longer need to insert a BR, it creates other issues.
                       Will remove this completely if no other issues arise from this. **/
                      /** @author alissa, otherwise on next selection the filling char will be removed and line will lose height */
//                        var anchorNode = range.startContainer.type == CKEDITOR.NODE_ELEMENT ? range.startContainer : null;
//                        var fillingChar;
//                        if(anchorNode && CKEDITOR.dtd.$block[anchorNode.getName()]){
//                            fillingChar = this.root.getDocument().createElement('br');
//                        } else{
//                            fillingChar = createFillingChar( this.root );
//                        }
